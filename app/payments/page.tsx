...
export default function PaymentsPage() {
  ...
  /* ΝΕΑ ΠΕΔΙΑ ΓΙΑ ΠΡΟΠΛΗΡΩΜΗ */
  const [isPrepayment, setIsPrepayment] = useState(false);
  const [prepaymentMonths, setPrepaymentMonths] =
    useState("2");
  ...
  async function loadData() {
    ...
    if (studentsError) {
      ...
      alert(
        "Δεν ήταν δυνατή η φόρτωση των μαθητών."
      );
    }
    ...
    if (paymentsError) {
      ...
      alert(
        "Δεν ήταν δυνατή η φόρτωση των πληρωμών."
      );
    }
    ...
    if (student) {
      ...
      if (isPrepayment) {
        const numberOfMonths =
          Number(prepaymentMonths) || 1;

        setAmount(
          String(fee * numberOfMonths)
        );
      } else {
  ...
  function handlePrepaymentChange(
    enabled: boolean
  ) {
    setIsPrepayment(enabled);
  ...
  function handlePrepaymentMonthsChange(
    value: string
  ) {
    setPrepaymentMonths(value);
  ...
  async function addPayment(
     ...
     * ΚΑΝΟΝΙΚΗ ΠΛΗΡΩΜΗ
     *
     * Δεν αλλάζει τίποτα από τον υπάρχοντα τρόπο.
     */
    if (!isPrepayment) {
      ...
      alert(
        "Η πληρωμή καταχωρήθηκε επιτυχώς."
      );
     ...
     * ΠΡΟΠΛΗΡΩΜΗ
     *
     * Δημιουργούμε μία ξεχωριστή εγγραφή
     * για κάθε μήνα.
     *
     * Οι ήδη υπάρχουσες πληρωμές δεν
     * τροποποιούνται.
     */

    const numberOfMonths =
      Number(prepaymentMonths);
    ...
    if (
      ...
      alert(
        "Για προπληρωμή επίλεξε από 2 έως 12 μήνες."
      );
      return;
    }
    ...
    if (monthlyFee <= 0) {
      alert(
        "Ο μαθητής δεν έχει ορισμένη μηνιαία χρέωση."
      );
      return;
    }

    const expectedTotal =
      monthlyFee * numberOfMonths;

    const enteredTotal = Number(amount);

    if (Math.abs(enteredTotal - expectedTotal) > 0.01) {
      alert(
        `Για ${numberOfMonths} μήνες το ποσό πρέπει να είναι ${expectedTotal.toFixed(
          2
        )}€.`
      );
      return;
    ...
    if (error) {
      ...
      alert(
        "Η προπληρωμή δεν καταχωρήθηκε."
      );

      return;
    }

    const startMonth = monthLabel(month);
    ...
    setPrepaymentMonths("2");

    await loadData();

    alert(
      `Η προπληρωμή καταχωρήθηκε επιτυχώς.\n\n${student.name}\n${startMonth} → ${endMonth}\n\nΣύνολο: ${formatMoney(
        expectedTotal
      )}`
    );
  }
  ...
  function receiptNumber(
    ...
    return String(payment.id).padStart(
      6,
      "0"
    );
  }
          ...
          <div className="mb-8 overflow-hidden rounded-3xl bg-white shadow-sm ring-1 ring-slate-200">
            ...
            <div className="border-b border-slate-100 px-6 py-5">
              ...
              <p className="mt-1 text-sm text-slate-500">
                Καταχώρησε κανονική πληρωμή ή προπληρωμή
              </p>
            ...
            >
              ...
              {isPrepayment ? (
                ...
                <div>
                  ...
                  <div className="rounded-xl border border-slate-200 bg-slate-50 p-3.5 text-sm font-medium text-slate-700">
                    {monthLabel(month)}
                  </div>
                </div>
              )}
              ...
              <div>
                ...
                {isPrepayment &&
                  studentId && (
                    <p className="mt-2 text-xs text-slate-500">
                      Το ποσό υπολογίζεται με βάση τη μηνιαία χρέωση του μαθητή.
                    </p>
                  )}
              ...
              {isPrepayment &&
                studentId && (
                  <div className="rounded-2xl border border-blue-100 bg-blue-50 p-5 md:col-span-2">
                        ...
                        (_, index) => {
                          ...
                          return (
                            <div
                              ...
                              className="flex justify-between rounded-xl bg-white/70 px-4 py-3"
                            >
                              <span>
                                {monthLabel(
                                  paymentMonth
                    ...
                    <div className="mt-4 flex justify-between border-t border-blue-200 pt-4 text-sm font-bold text-blue-900">
